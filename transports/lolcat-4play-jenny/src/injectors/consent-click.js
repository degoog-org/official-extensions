import { consentMatchers } from "../browser/consents.js";
...
export const consentClickJs = () => `(() => {
  ...
  const controls = [...document.querySelectorAll('button, input[type="submit"], input[type="button"], [role="button"], a[role="button"]')]
    .map((el) => ({ el, text: norm(el.innerText || el.value || el.getAttribute("aria-label") || el.textContent) }));
  ...
  if (!looksConsent) return { consent: false, progressed: false, href: location.href, title: document.title || "" };
  const rank = (text) => text === "accept all" ? 3 : text.startsWith("accept all") ? 2 : 1;
  const target = bySelector(isVisible) ||
    (acceptButtons.filter(({ el }) => isVisible(el)).sort((a, b) => rank(b.text) - rank(a.text))[0] || acceptButtons[0])?.el;
  if (target) {
    target.click();
    return { consent: true, progressed: true, via: "consent", label: norm(target.innerText || target.textContent || target.value) || "accept", href: location.href, title: document.title || "" };
  }
